...
class SearchVC: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate{
    ...
    func getData(text:String){
        ...
        let dictRequest = ["accesskey":90336,"type":"products-search","search":text] as [String : Any]
        print(dictRequest)
        
        AF.request(PRODUCTS_SEARCH, method:.post,parameters: dictRequest, headers:header).responseJSON { (response) in
...
//                    }
    ...
    func searchBarShouldBeginEditing(_ searchBar: UISearchBar) -> Bool {
        ...
        if searchBar.text!.count > 0{
            if searchBar.text!.count > 0{
                self.getData(text: searchBar.text!)
            }
        }else{
            self.product_list.removeAll()
            self.collectionView.reloadData()
